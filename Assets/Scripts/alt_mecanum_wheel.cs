...
public class alt_mecanum_wheel : MonoBehaviour
{
    public Rigidbody rb;
    ...
    void Start()
    {
        ...
        alpha = Vector3.SignedAngle(transform.parent.right, transform.right, Vector3.up) * Mathf.Deg2Rad;
        Debug.Log(transform.name + " alpha: "+ alpha * Mathf.Rad2Deg);
    ...
    void FixedUpdate()
    {
        ...
        matJ[0, 0] = - Mathf.Sin(alpha + beta + gamma);
        matJ[0, 1] = Mathf.Cos(alpha + beta + gamma);
        matJ[0, 2] = l * Mathf.Cos(beta + gamma);

        // calcualte velocity
        float[,] mat = MatrixMultiply(matROT, matVel);
        mat = MatrixMultiply(matJ, mat);
        vel = -1/(r * Mathf.Cos(gamma)) * mat[0,0];
